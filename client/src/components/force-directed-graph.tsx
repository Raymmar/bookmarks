...
export function ForceDirectedGraph({ bookmarks, insightLevel, onNodeClick }: ForceDirectedGraphProps): JSX.Element {
  ...
  const resetGraphView = useCallback(() => {
    ...
    console.log("Resetting graph view to show all nodes");
  ...
  // Listen for reset graph view events
  useEffect(() => {
    document.addEventListener('resetGraphView', resetGraphView);
    
    // Clean up event listener on unmount
    return () => {
      document.removeEventListener('resetGraphView', resetGraphView);
    };
  }, [resetGraphView]);
