...
export function useBookmarkMutations() {
  ...
  // Update an existing bookmark with optimistic updates
  const updateBookmark = useMutation({
    mutationFn: async ({ bookmarkId, updateData }: { bookmarkId: string, updateData: Partial<Bookmark> }) => {
      return apiRequest<Bookmark>("PATCH", `/api/bookmarks/${bookmarkId}`, updateData);
    },
    onMutate: async ({ bookmarkId, updateData }) => {
      // Cancel any outgoing refetches so they don't overwrite our optimistic update
      await queryClient.cancelQueries({ queryKey: ["/api/bookmarks"] });
      ...
      queryClient.setQueryData<Bookmark[]>(["/api/bookmarks"], (old = []) => {
        return old.map(bookmark => 
          bookmark.id === bookmarkId 
            ? { ...bookmark, ...updateData, last_updated: new Date().toISOString() } 
            : bookmark
        );
      });
      
      // If we have a specific bookmark query, update it too
      const previousBookmark = queryClient.getQueryData<Bookmark>([`/api/bookmarks/${bookmarkId}`]);
      if (previousBookmark) {
        queryClient.setQueryData<Bookmark>([`/api/bookmarks/${bookmarkId}`], {
          ...previousBookmark, 
          ...updateData,
    ...
    onError: (err, variables, context: any) => {
      ...
      if (context?.previousBookmark) {
        queryClient.setQueryData([`/api/bookmarks/${variables.bookmarkId}`], context.previousBookmark);
      }
    ...
    onSuccess: (updatedBookmark, variables) => {
      // First update the specific bookmark query
      queryClient.setQueryData([`/api/bookmarks/${variables.bookmarkId}`], updatedBookmark);
      
      // Then invalidate all bookmark queries to ensure data consistency
      queryClient.invalidateQueries({ queryKey: ["/api/bookmarks"] });
      queryClient.invalidateQueries({ queryKey: ["/api/activities"] });
      
      // Dispatch the bookmark update event
      const event = new CustomEvent('bookmarkUpdated', { 
        detail: {
          bookmarkId: variables.bookmarkId,
          updatedFields: variables.updateData,
          updatedBookmark
        }
      });
      ...
      toast({
        ...
        description: "Your bookmark was successfully updated",
      });
    }
  });
