...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/collections", async (req, res) => {
    try {
      // If user is authenticated, return their collections + public collections
      if (req.isAuthenticated()) {
        const userCollections = await storage.getUserCollections(req.user.id);
        const publicCollections = await storage.getPublicCollections();
        
        // Filter out duplicates (user's collections that are also public)
        const userCollectionIds = new Set(userCollections.map(c => c.id));
        const filteredPublicCollections = publicCollections.filter(c => !userCollectionIds.has(c.id));
        
        res.json([...userCollections, ...filteredPublicCollections]);
      } else {
        // If not authenticated, return only public collections
        const publicCollections = await storage.getPublicCollections();
  ...
  app.post("/api/collections", async (req, res) => {
    try {
      ...
      if (!req.isAuthenticated()) {
        return res.status(401).json({ error: "You must be logged in to create a collection" });
      }
  ...
  app.get("/api/collections/:id", async (req, res) => {
    try {
      ...
      // If the collection is not public and the user is not authenticated
      // or not the owner, deny access
      if (!collection.is_public && 
          (!req.isAuthenticated() || req.user.id !== collection.owner_id)) {
        // Check if user has membership access
        if (req.isAuthenticated()) {
          const memberships = await storage.getUserCollectionMemberships(req.user.id);
          const hasMembership = memberships.some(m => m.collection_id === collection.id);
  ...
  app.patch("/api/collections/:id", async (req, res) => {
    try {
      ...
      if (!req.isAuthenticated()) {
        return res.status(401).json({ error: "You must be logged in to update a collection" });
      }
      ...
      const updatedCollection = await storage.updateCollection(req.params.id, updateData);
  ...
  app.delete("/api/collections/:id", async (req, res) => {
    try {
      ...
      if (!req.isAuthenticated()) {
        return res.status(401).json({ error: "You must be logged in to delete a collection" });
      }
  ...
  // Collection membership endpoints
    ...
    try {
      ...
      if (req.isAuthenticated()) {
        if (req.user.id !== collection.owner_id) {
          ...
          if (!hasMembership) {
            return res.status(403).json({ error: "You don't have access to view members" });
          }
        }
      } else {
        return res.status(401).json({ error: "You must be logged in to view collection members" });
      }
      
      const members = await storage.getCollectionMembers(req.params.id);
      res.json(members);
    } catch (error) {
      console.error("Error fetching collection members:", error);
      res.status(500).json({ error: "Failed to retrieve collection members" });
    }
  });
  
  app.post("/api/collections/:id/members", async (req, res) => {
    try {
      // User must be authenticated to add members
      if (!req.isAuthenticated()) {
        return res.status(401).json({ error: "You must be logged in to add members" });
      }
      ...
      if (user.id === collection.owner_id) {
        return res.status(400).json({ error: "Cannot add owner as a member" });
      }
      
      // Add the member to the collection
      const membership = await storage.addUserToCollection(
        collection.id,
        user.id,
        role || "viewer"
      );
      ...
      res.status(500).json({ error: "Failed to add member to collection" });
    }
  });
  
  app.delete("/api/collections/:collectionId/members/:userId", async (req, res) => {
    try {
      // User must be authenticated to remove members
      if (!req.isAuthenticated()) {
        return res.status(401).json({ error: "You must be logged in to remove members" });
      }
      ...
      await storage.removeUserFromCollection(req.params.collectionId, req.params.userId);
      
      res.status(204).send();
    } catch (error) {
      console.error("Error removing collection member:", error);
      res.status(500).json({ error: "Failed to remove member from collection" });
    }
  });
  
  // Bookmark-collection relationship endpoints
  app.get("/api/collections/:id/bookmarks", async (req, res) => {
    try {
      ...
      const bookmarks = await storage.getBookmarksByCollectionId(req.params.id);
  ...
  app.post("/api/collections/:id/bookmarks", async (req, res) => {
    try {
      // User must be authenticated to add bookmarks to a collection
      if (!req.isAuthenticated()) {
        return res.status(401).json({ error: "You must be logged in to add bookmarks to a collection" });
      }
      ...
      if (req.user.id !== collection.owner_id) {
        const memberships = await storage.getUserCollectionMemberships(req.user.id);
        const membership = memberships.find(m => m.collection_id === collection.id);
        
        if (!membership || membership.role === "viewer") {
          return res.status(403).json({ error: "You don't have permission to add bookmarks to this collection" });
        }
      }
      ...
      const relationship = await storage.addBookmarkToCollection(bookmarkId, collection.id);
  ...
  app.delete("/api/collections/:collectionId/bookmarks/:bookmarkId", async (req, res) => {
    try {
      // User must be authenticated to remove bookmarks from a collection
      if (!req.isAuthenticated()) {
        return res.status(401).json({ error: "You must be logged in to remove bookmarks from a collection" });
      }
      ...
      if (req.user.id !== collection.owner_id) {
        ...
        if (!membership || membership.role === "viewer") {
          return res.status(403).json({ error: "You don't have permission to remove bookmarks from this collection" });
        }
      }
      
      await storage.removeBookmarkFromCollection(req.params.bookmarkId, req.params.collectionId);
