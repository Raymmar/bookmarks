...
export async function setupSchedulers(
  ...
  await setupXSyncScheduler(db, aiProcessorService, xService);
  setupAIProcessingScheduler(aiProcessorService);
}
...
export async function setupXSyncScheduler(
  ...
  cron.schedule('0 0,12 * * *', async () => {
    try {
      ...
      await syncAllXAccounts(db, xService);
 ...
 */
export function setupAIProcessingScheduler(aiProcessorService: AIProcessorService) {
 ...
 */
async function syncAllXAccounts(db: Db, xService: XService) {
  try {
