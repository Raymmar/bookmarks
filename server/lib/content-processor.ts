...
export class ContentProcessor {
   ...
   */
  public async processContent(contentHtml: string): Promise<ProcessedContent> {
...
Format your response as valid JSON with these exact keys:
{
   ...
   */
  private readonly TAG_SYSTEM_PROMPT = `You are an AI assistant that extracts relevant tags from content.
...
`;
   ...
   */
  public async generateTags(storage: Storage, content: string, url?: string, customSystemPrompt?: string): Promise<string[]> {
    try {
      // Check if this is an X.com URL
      const isXTweet = url && (url.includes('twitter.com') || url.includes('x.com'));
      ...
      let systemPrompt = this.TAG_SYSTEM_PROMPT;
...
Additional User Instructions: ${userSystemPrompt}`;
   ...
   * Summarize content
   */
  public async summarizeContent(storage: Storage, content: string, customSystemPrompt?: string): Promise<string> {
    try {
