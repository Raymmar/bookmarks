    ...
    try {
      ...
      if (bookmark) {
        ...
        if (
          isXTweet &&
          bookmark.media_urls &&
          Array.isArray(bookmark.media_urls) &&
...
`;
        ...
        console.log(
          `Created templated system prompts with bookmark context for ${bookmark.id}`,
        );
  ...
  async processAiBookmarkData(
    ...
    console.log(
      `Starting AI processing for bookmark ${bookmarkId} in background`,
    );
    ...
    try {
      ...
      if (!bookmark) {
        console.error(
          `Cannot process AI data - bookmark ${bookmarkId} not found`,
        );
        return;
      }
      ...
      if (!url) {
        console.error(
          `No URL available for bookmark ${bookmarkId}, skipping AI processing`,
        );
        return;
      }
      ...
      console.log(
        `Retrieved system prompts for AI processing with bookmark context`,
      );
      ...
      const [embedding, aiTags, insights] = await Promise.all([
        ...
        (async () => {
          try {
            ...
            console.log(
              `No content available for embedding generation, skipping for bookmark ${bookmarkId}`,
            );
            return null;
          } catch (error) {
            console.error(
              `Error generating embedding for bookmark ${bookmarkId}:`,
              error,
            );
            return null;
          }
        ...
        (async () => {
          try {
            console.log(
              `Generating AI tags for bookmark ${bookmarkId} using URL: ${url}`,
            );
            ...
            if (isXTweet && bookmark.description) {
              console.log(
                `Using X tweet description as primary content for tag generation: "${bookmark.description.substring(0, 50)}..."`,
              );
            ...
            console.error(
              `Error generating AI tags for bookmark ${bookmarkId}:`,
              error,
            );
            return [];
          }
        ...
        (async () => {
          try {
            console.log(
              `Generating insights for bookmark ${bookmarkId} with depth ${insightDepth} using URL: ${url}`,
            );
            let processedText = "";
            ...
            if (isXTweet && bookmark.description) {
              console.log(
                `Using X tweet description as primary content for insights generation: "${bookmark.description.substring(0, 50)}..."`,
              );
              processedText = bookmark.description;
            }
            ...
            if (
              ...
              mediaUrls = bookmark.media_urls.filter(
                (url: string) =>
              ...
              if (mediaUrls.length > 0) {
                console.log(
                  `Found ${mediaUrls.length} Twitter image URLs to include in the analysis`,
                );
              }
            }
            ...
            const result = await generateInsights(
              ...
              processedText || "",
              insightDepth,
              systemPrompts.summaryPrompt ?? undefined,
              mediaUrls.length > 0 ? mediaUrls : undefined,
            );
            console.log(
              `Insights generated for bookmark ${bookmarkId}. Summary length: ${result.summary.length}`,
            );
            return result;
          } catch (error) {
            console.error(
              `Error generating insights for bookmark ${bookmarkId}:`,
              error,
            );
            return null;
          }
      ...
      if (embedding && embedding.length > 0) {
        try {
          ...
          console.error(
            `Error updating bookmark ${bookmarkId} with embedding:`,
            error,
          );
        }
      }
          ...
          try {
            ...
            if (!tagExists) {
              ...
              if (!tag) {
                ...
                console.log(
                  `Created new system tag "${tagName}" for bookmark ${bookmarkId}`,
                );
              }
              ...
              console.log(
                `Associated tag "${tagName}" with bookmark ${bookmarkId}`,
              );
            }
          } catch (tagError) {
            console.error(
              `Error adding AI tag "${tagName}" to bookmark ${bookmarkId}:`,
              tagError,
            );
          }
        }
      ...
      if (insights) {
        try {
          ...
          const existingInsight =
            await this.storage.getInsightByBookmarkId(bookmarkId);

          if (existingInsight) {
            console.log(
              `Updating existing insights for bookmark ${bookmarkId}`,
            );
          ...
          console.log(
            `Insights and activity created for bookmark ${bookmarkId}`,
          );
        } catch (error) {
          console.error(
            `Error storing insights for bookmark ${bookmarkId}:`,
            error,
          );
        }
      }
  ...
  async createBookmark(options: BookmarkCreationOptions) {
    ...
    if (
      urlResult.exists &&
      urlResult.existingBookmarkId &&
      urlResult.existingForUser
    ) {
      // URL already exists for this user - update the existing bookmark with new information
      const existingBookmark = await this.storage.getBookmark(
        urlResult.existingBookmarkId,
      );
      console.log(
        `URL already exists as bookmark for this user: ${urlResult.existingBookmarkId} - Updating with new information`,
      );
      ...
      if (
        ...
        options.content_html !== existingBookmark.content_html
      ) {
      ...
      if (Object.keys(updateData).length > 0) {
        await this.storage.updateBookmark(
          urlResult.existingBookmarkId,
          updateData,
        );
        console.log(
          `Updated bookmark ${urlResult.existingBookmarkId} with new information`,
        );
      }
      ...
      if (options.tags && options.tags.length > 0) {
        try {
          ...
          const existingTags = await this.storage.getTagsByBookmarkId(
            urlResult.existingBookmarkId,
          );
          ...
          for (const tagName of formattedTags) {
            ...
            if (existingTagNames.includes(tagName.toLowerCase())) {
              console.log(
                `Tag "${tagName}" already exists on bookmark ${urlResult.existingBookmarkId}, skipping`,
              );
              continue;
            }
            ...
            // Associate tag with bookmark
            await this.storage.addTagToBookmark(
              urlResult.existingBookmarkId,
              tag.id,
            );
            ...
            await this.storage.incrementTagCount(tag.id);
            console.log(
              `Added new tag "${tagName}" to existing bookmark ${urlResult.existingBookmarkId}`,
            );
          }
        } catch (error) {
          console.error(
            `Error adding new tags to existing bookmark ${urlResult.existingBookmarkId}:`,
            error,
          );
        }
      }
      ...
      const updatedBookmark = await this.storage.getBookmark(
        urlResult.existingBookmarkId,
      );
    ...
    const bookmarkData: InsertBookmark = {
      ...
      title:
        options.title || urlResult.normalized.split("/").pop() || "Untitled",
    ...
    if (bookmarkData.url && (!options.title || !options.description)) {
      try {
        ...
        bookmarkData.title =
          options.title || metadata.title || bookmarkData.title;
        bookmarkData.description =
          options.description ||
          metadata.description ||
          bookmarkData.description;
        bookmarkData.content_html =
          metadata.content || bookmarkData.content_html;
      } catch (error) {
    ...
    if (options.tags && options.tags.length > 0) {
      try {
        ...
        console.log(
          `Normalizing ${options.tags.length} user tags for bookmark ${bookmark.id}`,
        );
        const normalizedTags = processAITags(options.tags);
        console.log(`Normalized tags: ${normalizedTags.join(", ")}`);

        console.log(
          `Adding ${normalizedTags.length} normalized user tags to bookmark ${bookmark.id}`,
        );
        ...
        console.error(
          `Error adding user tags to bookmark ${bookmark.id}:`,
          error,
        );
      }
    }
    ...
    if (options.autoExtract) {
      ...
      if (!contentHtml || contentHtml.length === 0) {
        console.warn(
          `No content HTML available for bookmark ${bookmark.id}, fetching content now`,
        );
        ...
        try {
          ...
          if (metadata.content && metadata.content.length > 0) {
            console.log(
              `Successfully fetched content for ${bookmark.id}, length: ${metadata.content.length}`,
            );
            ...
            this.processAiBookmarkData(
              ...
              options.insightDepth
                ? typeof options.insightDepth === "string"
                  ? parseInt(options.insightDepth)
                  : options.insightDepth
                : 1,
            ).catch((error) => {
              console.error(
                `Background AI processing failed for bookmark ${bookmark.id}:`,
                error,
              );
            });

            console.log(
              `Started background AI processing for bookmark ${bookmark.id}`,
            );
          } else {
          ...
          console.error(
            `Error fetching content for AI processing of bookmark ${bookmark.id}:`,
            error,
          );
        }
      } else {
        ...
        console.log(
          `Processing bookmark ${bookmark.id} with existing HTML content (${contentHtml.length} chars)`,
        );
      ...
      console.log(
        `Skipping AI processing for bookmark ${bookmark.id} (autoExtract: ${options.autoExtract})`,
      );
    }
  ...
  async updateBookmark(
    bookmarkId: string,
    updateData: Partial<BookmarkCreationOptions>,
  ) {
    // Check if bookmark exists
    const bookmark = await this.storage.getBookmark(bookmarkId);
    if (!bookmark) {
    ...
    if (updateData.description)
      bookmarkUpdateData.description = updateData.description;
    if (updateData.user_id !== undefined)
      bookmarkUpdateData.user_id = updateData.user_id;
    ...
    const updatedBookmark = await this.storage.updateBookmark(
      bookmarkId,
      bookmarkUpdateData,
    );
    ...
    if (updateData.tags) {
      ...
      // Normalize the user-provided tags
      console.log(
        `Normalizing ${updateData.tags.length} user tags for bookmark ${bookmarkId}`,
      );
      ...
      for (const tagName of normalizedTags) {
        ...
        if (currentTagNames.includes(tagName.toLowerCase())) {
          console.log(
            `Tag "${tagName}" already exists on bookmark ${bookmarkId}, skipping`,
          );
          continue;
        }
